...
def _run(argv: list[str], workdir: Path | None = None) -> tuple[int, str]:
    """The gate, with its report captured. Nothing here writes to the real stdout."""
    buffer = StringIO()
    code = gate.main(argv, out=buffer, workdir=workdir)
...
def test_smoke_exits_zero_and_names_p4_for_everything_it_could_not_check(
    tmp_path: Path,
) -> None:
    """16-roadmap.md:458's exit criterion, and the "never a silent pass" half of it.

    Exit 0 is what P2 asks for. What makes that honest rather than empty is the NOT CHECKED
    block: four rows, each naming the phase that owns it, so a reader cannot mistake a harness
    for a gate.
    """
    ...
    assert output.count("owner: P4") == len(gate.NOT_CHECKED_AT_P2)
    assert "16-roadmap.md:550" in output
    assert "fixtures/incremental/" in output
    for expected in ("op.converge", "anchor_delta", "full rebuild"):
...
def test_the_full_mode_did_not_run_without_a_fixture(tmp_path: Path) -> None:
    ...
    assert "16-roadmap.md:458" in output
...
def test_a_correct_fixture_with_no_builder_still_does_not_pass(tmp_path: Path) -> None:
    """THE CASE THIS HARNESS EXISTS TO GET RIGHT.

    W4.10 lands the corpus; 08-runtime.md section 5.6 lands the indexer that would be driven
    through it. If they land in that order, a harness that read the plan, found the numbers
    correct and returned 0 would report G19 green over a gate that had never run -- and G19 is
    `pr = true`, so that green would be a required check passing on nothing. Exit 2.
    """
    fixture = _plan(tmp_path / "corpus", documents=300, mutations=40)
    code, output = _run(["--fixture", str(fixture)])
    assert code == gate.EXIT_NOT_RUN
    assert "nothing in this tree can index them" in output
    assert "G19 ok" not in output
