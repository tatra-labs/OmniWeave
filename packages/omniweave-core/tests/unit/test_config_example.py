...
# The declared patterns instantiate to EXACTLY ONE key each in this file: every glob is given
# exactly one instance, and `[corpora.contracts]` is a COMMENTED illustration rather than a second
# live instance (see `test_the_example_declares_one_instance_per_glob`). Pinned as a number because
# a key added to the registry without a line here, or a line here deleted, must fail LOUDLY rather
...
def test_the_example_instantiates_every_declared_pattern_once() -> None:
    """One key per pattern. A count, so a silent drift in either direction fails here too."""
...
def test_the_example_resolves_through_the_loader_with_nothing_left_to_a_default(
    ...
    """Every resolved value's source is THIS FILE, and there are `EXPECTED_KEYS` of them.
