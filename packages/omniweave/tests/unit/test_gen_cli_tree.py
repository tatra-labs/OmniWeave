...
def test_the_arity_rule_is_reported_and_not_raised() -> None:
    ...
    #  `skills` has two of its seven rostered verbs (W7.6b): schedule, like `doc`, not a breach.
    assert unrostered_groups() == ("doc (2)", "hooks (1)", "skills (2)")
    assert _grammar_failures() == ()
...
def test_a_group_advertises_the_verbs_it_has_and_not_the_ones_it_is_owed() -> None:
    ...
    assert GROUP_HELP == {"doc": "diff · grid", "hooks": "check", "skills": "install · remove"}
